...
int main()
{
	...
	for (unsigned int i = 0; i < sizeof(writeThreadId) / sizeof(writeThreadId[0]); ++i)
	{
		...
		if ((pthread_create(&writeThreadId[i], NULL, write, &writeThreadParam[i])) == -1)
		{
	...
	for (unsigned int i = 0; i < sizeof(readThreadId) / sizeof(readThreadId[0]); ++i)
	{
		if ((pthread_create(&readThreadId[i], NULL, read, NULL)) == -1)
		{
			printf("pthread_create error");
			return 1;
		}
	...
	for (unsigned int i = 0; i < sizeof(writeThreadId) / sizeof(writeThreadId[0]); ++i)
	{
		if (pthread_join(writeThreadId[i], NULL))
		{
	...
	for (unsigned int i = 0; i < sizeof(readThreadId) / sizeof(readThreadId[0]); ++i)
	{
		if (pthread_join(readThreadId[i], NULL))
		{
			printf("thread is not exit...\n");
			return 1;
		}
